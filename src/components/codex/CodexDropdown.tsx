...
import { getCodexVersion } from "@/services/providerVersion";

const CODEX_LINKS = {
  dashboard: "https://platform.openai.com",
  docs: "https://openai.github.io/codex-cli",
  status: "https://status.openai.com",
};

export function CodexDropdown() {
  ...
  return (
    <div ref={dropdownRef} className="relative">
      <button
        ...
        title="Codex Manager"
      >
        <img src="/openai-white.svg" alt="Codex" className="w-4 h-4" />
      </button>
        ...
        <div className="absolute right-0 mt-1 w-64 py-1 bg-bg-secondary border border-border rounded-lg shadow-xl z-50 dropdown-solid">
          <div className="px-3 py-2 border-b border-border">
            <div className="flex items-center justify-between">
              <span className="text-sm font-medium text-text-primary">Codex CLI</span>
          ...
          <div className="py-1">
            <button
              onClick={() => handleOpenLink(CODEX_LINKS.dashboard)}
            ...
            <button
              onClick={() => handleOpenLink(CODEX_LINKS.docs)}
              className="w-full flex items-center gap-2 px-3 py-2 text-sm text-text-secondary hover:bg-bg-hover hover:text-text-primary transition-colors"
            >
            ...
            <button
              onClick={() => handleOpenLink(CODEX_LINKS.status)}
              className="w-full flex items-center gap-2 px-3 py-2 text-sm text-text-secondary hover:bg-bg-hover hover:text-text-primary transition-colors"
            >
