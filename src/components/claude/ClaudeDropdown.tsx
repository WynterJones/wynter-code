...
export function ClaudeDropdown({ projectPath }: ClaudeDropdownProps) {
  const {
    isDropdownOpen,
    toggleDropdown,
    closeDropdown,
  ...
  const handleItemClick = (tab: "commands" | "skills" | "subagents" | "settings") => {
    setActiveTab(tab);
    openPopup();
    closeDropdown();
  };
        ...
        <div className="absolute right-0 mt-1 w-64 py-1 bg-bg-secondary border border-border rounded-lg shadow-xl z-50 dropdown-solid">
          <div className="px-3 py-2 border-b border-border">
            <div className="flex items-center justify-between">
              ...
              <span className="text-xs text-text-secondary">v{versionInfo.current}</span>
            </div>
            <button
              onClick={(e) => {
                e.stopPropagation();
          ...
          <div className="py-1">
            <button
              ...
              className="w-full flex items-center justify-between px-3 py-2 text-sm text-text-secondary hover:bg-bg-hover hover:text-text-primary transition-colors"
            >
              <div className="flex items-center gap-2">
                <Terminal className="w-4 h-4" />
                <span>Commands</span>
              </div>
              <span className="text-xs text-text-secondary/60">{commands.length}</span>
            </button>
            ...
            >
              ...
              <span className="text-xs text-text-secondary/60">{skills.length}</span>
            </button>

            <button
              onClick={() => handleItemClick("subagents")}
              className="w-full flex items-center justify-between px-3 py-2 text-sm text-text-secondary hover:bg-bg-hover hover:text-text-primary transition-colors"
            >
              <div className="flex items-center gap-2">
              ...
              <span className="text-xs text-text-secondary/60">{subagents.length}</span>
            </button>
          ...
          <div className="border-t border-border pt-1">
            <button
              ...
              className="w-full flex items-center justify-center gap-2 px-3 py-2 text-sm text-accent hover:bg-accent/10 transition-colors"
            >
              <ExternalLink className="w-4 h-4" />
              <span>Open Claude Manager</span>
            </button>
          </div>
        </div>
      )}
