    ...
    <>
      ...
      <div className="flex h-full gap-4">
        {columns.map((column) => (
          <div
            key={column.id}
            className="relative flex min-w-0 flex-1 flex-col rounded-lg border border-border bg-bg-secondary"
          >
            {/* Column Header */}
...
function TestingSettingsPopup({ settings, onUpdate, onClose }: TestingSettingsPopupProps) {
  return (
    <>
      ...
      {/* Popup */}
      <div className="absolute right-2 top-11 z-20 w-56 rounded-md border border-border bg-[#1a1a1a] p-2 shadow-xl">
        {/* Concurrent Workers */}
        <div className="mb-2 px-1">
          <div className="mb-1 text-[11px] font-medium text-text-secondary">Workers</div>
          <div className="flex items-center gap-2">
            <input
              ...
              className="flex-1 h-1 rounded bg-bg-tertiary accent-accent cursor-pointer"
            />
            <span className="w-5 text-center text-xs font-medium">{settings.maxConcurrentIssues}</span>
          </div>
        </div>

        {/* Verification Steps */}
        <div className="border-t border-border/50 pt-2 mt-2">
          <div className="mb-1 px-1 text-[11px] font-medium text-text-secondary">Verification</div>
          <div className="flex flex-col gap-0.5">
            <label className="flex cursor-pointer items-center gap-2 rounded px-1.5 py-1 text-xs transition-colors hover:bg-white/5">
              <input
                type="checkbox"
                checked={settings.runLint}
                onChange={(e) => onUpdate({ runLint: e.target.checked })}
                className="h-3 w-3 rounded border-border bg-bg-secondary accent-accent"
              />
              <span>Lint</span>
            </label>
            <label className="flex cursor-pointer items-center gap-2 rounded px-1.5 py-1 text-xs transition-colors hover:bg-white/5">
        ...
        <div className="border-t border-border/50 pt-2 mt-2">
          <label className="flex cursor-pointer items-center gap-2 rounded px-1.5 py-1 text-xs transition-colors hover:bg-white/5">
            <input
              ...
              className="h-3 w-3 rounded border-border bg-bg-secondary accent-accent"
            />
            <span>Skip unrelated failures</span>
          </label>
        </div>
      </div>
    </>
