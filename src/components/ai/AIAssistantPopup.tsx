    ...
    <div className={`flex ${isUser ? "justify-end" : "justify-start"} mb-3`}>
      ...
      >
        {isUser ? (
          ...
          <div className="ai-assistant-markdown [&_pre]:!bg-bg-primary [&_pre]:!border-border [&_.relative]:!bg-bg-primary [&_.relative]:!my-2">
...
export function AIAssistantPopup({
  ...
  const {
    ...
    stopSession,
    clearMessages,
  } = useAIAssistant(sessionId, systemContext);

  // Manage session lifecycle based on popup open/close
  useEffect(() => {
    if (isOpen) {
      // Start session when popup opens
      startSession().catch((err) => {
        console.error("[AIAssistantPopup] Failed to start session:", err);
      });
    } else {
  ...
  const handleSubmit = useCallback(async () => {
    ...
    try {
      ...
      console.error("[AIAssistantPopup] Failed to send prompt:", error);
    }
  }, [input, isStreaming, isSessionActive, sendPrompt]);

  // Handle escape key on the container
  ...
  // Handle enter key on the textarea
        ...
        <div className="p-4 border-t border-border shrink-0">
          <div className="flex gap-2">
            <textarea
              ...
              placeholder={isSessionActive ? placeholder : "Connecting..."}
            ...
            <button
              ...
              className="self-start px-3 py-2 bg-accent text-primary-950 rounded-md hover:bg-accent/90 disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
            >
              {isStreaming ? (
